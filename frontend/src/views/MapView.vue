...
import { getMapLocations, getMapPopularPosts } from '../api/mapApi'
import { loadKakaoMap, type KakaoMaps } from '../utils/kakaoMapLoader'
import type { MapLocation, MapPopularPost } from '../types/map'
...
const popularPosts = ref<MapPopularPost[]>([])
...
const selectLocation = async (location: MapLocation) => {
  ...
  try {
    const response = await getMapPopularPosts(location.id)
    if (requestId === selectedRequestId) popularPosts.value = response.data.items
  } finally {
    if (requestId === selectedRequestId) popularLoading.value = false
  }
}
...
const updateMarkers = async () => {
  ...
  try {
    const response = await getMapLocations({
      minLat: southWest.getLat(), maxLat: northEast.getLat(),
      minLng: southWest.getLng(), maxLng: northEast.getLng(),
      category: category.value, signal: controller.signal,
    })
    ...
    const markers = locations.map((location) => {
      ...
      kakao.event.addListener(marker, 'click', () => { void selectLocation(location) })
      return marker
    })
  ...
  <main class="bg-gray-50 py-8">
    <section class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="mb-5 flex flex-col gap-4 rounded-2xl bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div><h1 class="text-2xl font-semibold text-gray-900">서울 관광 지도</h1><p class="mt-1 text-sm text-gray-600">지도를 움직여 주변 관광지와 맛집을 찾아보세요.</p></div>
      ...
      <div class="grid gap-5 lg:grid-cols-[1fr_320px]">
        ...
        <aside class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <template v-if="selected">
            <img v-if="selected.image_url" :src="selected.image_url" :alt="selected.title" class="mb-4 h-44 w-full rounded-xl object-cover" />
            <div v-else class="mb-4 flex h-44 items-center justify-center rounded-xl bg-gray-100 text-sm text-gray-500">이미지 없음</div>
            <h2 class="text-xl font-semibold text-gray-900">{{ selected.title }}</h2><p class="mt-3 text-sm leading-6 text-gray-600">{{ selected.address || '주소 정보 없음' }}</p>
            <div class="mt-6 border-t pt-4"><h3 class="font-semibold text-gray-900">이 장소의 인기 게시글</h3><p v-if="popularLoading" class="mt-3 text-sm text-gray-500">게시글을 불러오는 중...</p><p v-else-if="!popularPosts.length" class="mt-3 text-sm text-gray-500">등록된 게시글이 없습니다.</p><template v-else><RouterLink v-for="post in popularPosts" :key="post.id" :to="{ name: 'post-detail', params: { id: String(post.id) } }" class="mt-3 block rounded-lg bg-gray-50 p-3 hover:bg-sky-50"><p class="truncate text-sm font-medium text-gray-900">{{ post.title }}</p><div class="mt-1 flex gap-2 text-xs text-gray-500"><span>조회 {{ post.view_count }}</span><span>댓글 {{ post.comment_count }}</span><span>좋아요 {{ post.like_count }}</span></div></RouterLink></template></div>
          </template>
          <div v-else class="flex h-full min-h-48 items-center justify-center text-center text-sm text-gray-500">지도 위의 마커를 클릭하면<br />사진, 이름, 주소를 확인할 수 있습니다.</div>
        </aside>
      </div>
    </section>
  </main>
