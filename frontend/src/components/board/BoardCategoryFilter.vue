...
import type { PostCategory } from '../../types/board'

interface Props {
  modelValue?: PostCategory
}
...
const emit = defineEmits<{
  (e: 'update:modelValue', value: PostCategory): void
}>()

const filters: Array<{ value: PostCategory; label: string }> = [
  { value: 'all', label: '전체' },
  { value: 'free', label: '자유' },
    ...
    <div class="flex flex-wrap gap-2 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      <button
        ...
        :class="modelValue === filter.value
          ...
          : 'border-gray-300 text-gray-600 hover:border-sky-500 hover:text-sky-600'"
